...
object Day03:
  ...
  private def tryParse(chars: Seq[Char], i: Int, s: String): Option[Int] =
    ...
    if res.size == s.size then Some(i + s.size)
    else None

  private def tryParse(chars: Seq[Char], i: Int, s: Seq[Char]): Seq[Char] =
    if chars.size - i < s.size then Seq.empty
    else if chars(i) == s.head then
      val res = chars(i)
      if s.size > 1 then chars(i) +: tryParse(chars, i + 1, s.drop(1))
      else Seq(chars(i))
    else Seq.empty
  ...
  private def tryParseNum(chars: Seq[Char], i: Int): Option[(Long, Int)] =
    var digits = Seq[Char]()
    var j = i
    while j < chars.size && chars(j).isDigit && digits.size < 3 do
